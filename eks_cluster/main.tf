...
resource "aws_ec2_tag" "public_subnets" {
  ...
  key         = "kubernetes.io/cluster/${local.environment}-${local.service}"
  value       = "shared"
}#This is a rule to allow Istio traffict
resource "aws_security_group" "eks_nodes_sg" {
  ...
  ingress {
    ...
    to_port     = 1507
    protocol    = "tcp"
    cidr_blocks = [data.aws_vpc.vpc.cidr_block] # Replace with your CIDR range
  }
}
...
module "eks" {
  ...
  node_security_group_additional_rules = {
    ingress_nodes_Istio_port = {
      description                   = "Cluster API to Node group for Istio webhook"
      protocol                      = "tcp"
      from_port                     = 15017
      to_port                       = 15017
      type                          = "ingress"
...
#uncomment the following section to add the kubeflow components
